...
func TestParseImageURL_InvalidReference_ReturnsError(t *testing.T) {
	cases := []struct {
		...
		{name: "unqualified name without domain", input: "ubuntu"},
		{name: "unqualified name with tag", input: "nginx:latest"},
		{name: "namespace-only shorthand", input: "library/ubuntu"},
		{name: "org/repo shorthand", input: "org/repo"},
		{name: "org/repo shorthand with tag", input: "org/repo:v1"},
		{name: "team/image shorthand with tag", input: "team/image:v1"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := ParseImageURL(tc.input)
			if err == nil {
...
func TestGetImageRegistryAndRepository(t *testing.T) {
	cases := []struct {
		...
		{"plain registry/repo", "registry.io/repo", "registry.io/repo"},
		{"strips tag", "registry.io/repo:v1", "registry.io/repo"},
		{"strips tag and digest", "registry.io:5000/repo:v1@" + validDigest, "registry.io:5000/repo"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
...
func TestGetImageRegistryRepositoryTag(t *testing.T) {
	cases := []struct {
		...
		{"no tag returns bare repo", "registry.io/repo", "registry.io/repo"},
		{"includes tag", "registry.io/repo:v1", "registry.io/repo:v1"},
		{"strips digest keeps tag", "registry.io:5000/repo:v1@" + validDigest, "registry.io:5000/repo:v1"},
		{"digest only returns bare repo", "registry.io:5000/repo@" + validDigest, "registry.io:5000/repo"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
...
func TestGetImageRegistryRepositoryDigest(t *testing.T) {
	cases := []struct {
		...
		{"no digest returns bare repo", "registry.io/repo", "registry.io/repo"},
		{"includes digest", "registry.io/repo@" + validDigest, "registry.io/repo@" + validDigest},
		{"strips tag keeps digest", "registry.io:5000/repo:v1@" + validDigest, "registry.io:5000/repo@" + validDigest},
		{"tag only returns bare repo", "registry.io/repo:v1", "registry.io/repo"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
