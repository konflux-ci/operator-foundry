...
provided, it is treated as the catalog directory for the package (relative to
`--build-context`) and `lifecycle.json` is injected directly into it. Use this
when all `COPY` instructions targeting `/configs` use `--from=<stage>`.
