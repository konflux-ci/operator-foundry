...
import (
	...
	"github.com/konflux-ci/operator-foundry/pkg/mediatype"
...
with the condition that OCI mediaType is not present for OCP < v4.21.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fetcher := &mediatype.RegistryFetcher{}
			result, err := mediatype.CheckRelatedImagesMediaType(cmd.Context(), ocpVersion, relatedImages, fetcher)
			if err != nil {
				return err
			}
			...
			if result.Skipped {
				output, err = testoutput.MakeResultJSON(
					...
					testoutput.WithNote("Check skipped: OCP version supports OCI natively."),
				)
			} else if len(result.WrongMediaTypeImages) == 0 && len(result.BrokenImages) == 0 {
				output, err = testoutput.MakeResultJSON(
					testoutput.ResultSuccess,
					testoutput.WithSuccesses(1),
				...
				if len(note) > tektonOutputNoteLimit {
					truncated := note[:tektonOutputNoteLimit]
...
// loadRelatedImages reads the JSON file at path and returns the image reference strings it contains.
func loadRelatedImages(path string) ([]string, error) {
	data, err := os.ReadFile(path)
	if err != nil {
		return nil, fmt.Errorf("failed to read related images JSON from %q: %w", path, err)
	}
	var images []string
	if err := json.Unmarshal(data, &images); err != nil {
		return nil, fmt.Errorf("failed to parse related images JSON from %q: %w", path, err)
	}
	if images == nil {
